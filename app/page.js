...
export default function Home(){
  ...
  function add(p){
    trackEvent('add_to_cart',{currency:'PKR',value:PRICE,items:[{item_name:p.name,item_id:p.slug,price:PRICE,quantity:1}]});
  ...
  async function submitContact(e){
    ...
    setContactStatus({state:'sending',message:'Sending your message…'});
    try{
      const response=await fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:data.get('name'),email:data.get('email'),phone:data.get('phone'),message:data.get('message'),website:data.get('website')})});
      const result=await response.json();
      if(!response.ok) throw new Error(result.error||'We could not send your message.');
      formElement.reset();
      trackEvent('generate_lead',{method:'contact_form'});
      setContactStatus({state:'success',message:'Thank you. Your message has been sent to Qambranis.'});
    }catch(error){
      setContactStatus({state:'error',message:error.message||'We could not send your message. Please use WhatsApp.'});
    }
  }
  ...
  return <main>
    ...
    <header className="nav">
      <a className="wordmark" href="#top" aria-label="Qambranis home">QAMBRANIS<span>Empowering Women</span></a>
    ...
    <section id="top" className="hero">
      ...
      <div className="shade"/>
      <div className="heroCopy brandHero"><p>Handmade in Interior Sindh</p><h1>QAMBRANIS</h1><h2>Wear the story.</h2><a href="#collection" className="lightButton">Shop the Collection</a></div>
    ...
    <section id="collection" className="collection sectionPad">
      ...
      <div className="grid">
        {visibleProducts.map(p=><article key={p.slug} className="card">
          <button className="imageWrap" onClick={()=>{setSelected(p);setActiveImage(1);trackEvent('view_item',{currency:'PKR',value:PRICE,items:[{item_name:p.name,item_id:p.slug,price:PRICE}]})}}><Image src={`/images/products/${p.slug}-1.webp`} alt={p.name} fill sizes="(max-width:760px) 50vw, (max-width:1100px) 50vw, 33vw" className="cover"/></button>
          <div className="cardText"><div><span className="productLabel">Handmade · Made to order</span><h3>{p.name}</h3><p>{p.tone}</p></div><span className="cardPrice">{money(PRICE)}</span></div>
    ...
    <section className="editorial">{[1,2,3,4].map(i=><div className="editorialImg" key={i}><Image src={`/images/editorial/model-${i}.jpeg`} alt="Qambranis editorial" fill sizes="(max-width:760px) 50vw, (max-width:1100px) 50vw, 33vw" className="cover"/></div>)}</section>
