    ...
    <section id="collection" className="collection sectionPad">
      ...
      <div className="grid">
        {visibleProducts.map(p=><article key={p.slug} className="card">
          <button className="imageWrap" onClick={()=>{setSelected(p);setActiveImage(1);trackEvent('view_item',{currency:'PKR',value:PRICE,items:[{item_name:p.name,item_id:p.slug,price:PRICE}]})}}><Image src={`/images/products/${p.slug}-1.webp`} alt={p.name} fill sizes="(max-width:760px) 100vw,50vw" className="cover"/></button>
          ...
          <div className="cardActions"><button className="primaryAction" onClick={()=>{setSelected(p);setActiveImage(1);trackEvent('view_item',{currency:'PKR',value:PRICE,items:[{item_name:p.name,item_id:p.slug,price:PRICE}]})}}>View details</button><button onClick={()=>add(p)}>Add to bag</button></div>
        </article>)}
      </div>
